...
function sanitizeReview(value: unknown): VisualReview {
  ...
  const observations = Array.isArray(input.observations) ? input.observations.filter((x): x is string => typeof x === "string").slice(0, 6) : [];
  const nextSteps = Array.isArray(input.next_steps) ? input.next_steps.filter((x): x is string => typeof x === "string").slice(0, 5) : [];
...
export default async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (!isValidDataUrl(imageDataUrl)) return json({ error: "Foto tidak valid. Gunakan JPG, PNG, atau WebP maksimal 5 MB setelah kompresi." }, 400);

    const context = body.context && typeof body.context === "object" ? body.context : {};
    const contextRecord = context as Record<string, unknown>;
    const symptoms = Array.isArray(contextRecord.symptoms) ? contextRecord.symptoms.filter((x): x is string => typeof x === "string").slice(0, 10) : [];
    ...
    const prompt = [
      ...
      'Kembalikan JSON valid saja: {"photo_quality":"good|fair|poor","status":"Sehat|Perlu Diamati|Perlu Pemeriksaan|Data Belum Cukup","confidence":0,"observations":["..."],"caution":"...","next_steps":["..."]}',
    ...
    if (!raw) return json({ error: "Model tidak mengembalikan hasil pemeriksaan." }, 502);
    let parsed: unknown;
    try { parsed = JSON.parse(raw); } catch { return json({ error: "Format hasil AI tidak valid." }, 502); }
